...
class LedTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.led_display = LED(wet_value=24000, dry_value=52000)
    
    def testColorTransitions(self): 
        # Test 1: Simulate Bone Dry (Should be Red)
        logging.info("Simulating DRY soil...")
        self.led_display.updateLedColor(52000)
        self.assertAlmostEqual(self.led_display.moisture_perc, 0.0)
        time.sleep(1)

        # Test 2: Simulate Ideal Moisture (Should be Green)
        logging.info("Simulating PERFECT soil...")
        self.led_display.updateLedColor(38000)
        self.assertTrue(0.4 < self.led_display.moisture_perc < 0.6)
        time.sleep(1)

        # Test 3: Simulate Submerged (Should be Blue)
        logging.info("Simulating WET soil...")
        self.led_display.updateLedColor(24000)
        self.assertAlmostEqual(self.led_display.moisture_perc, 1.0)
        time.sleep(1)
    ...
    def testSweep(self):
        ...
        for val in range(52000, 24000, -1000):
            self.led_display.updateLedColor(val)
            time.sleep(0.1)